...
using static SingleDataBase.Features.Deals.GetInfo.DealInfoView;

namespace SingleDataBase.Features.Deals.GetInfo;
    ...
    public async Task<Result<DealInfoView>> Handle(GetDealInfoRequest request, CancellationToken cancellationToken)
    {
        ...
        return new DealInfoView(
           deal.Notes,
            deal.Status,
            deal.DeliveryDate,
             productInfo.Select(p => new ProductInfoView(
                 p.Name,
                 p.ImageUrl,
                 p.Description,
                 p.Price,
